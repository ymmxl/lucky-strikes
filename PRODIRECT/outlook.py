...
def main(maxthread,emails):
	ctypes.windll.kernel32.SetConsoleTitleW(f"Threads: {maxthread} | Entered: 0 | Failed: 0")
	q = queue.Queue()
	for i in emails:
		q.put(i)
	try:
		for i in range(0,maxthread):
			Thread(target=process,args=(maxthread,q)).start()

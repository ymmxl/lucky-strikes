...
async def calculateDimensions(q,qtype,optcount):
	dimension = []
...
async def calculateRelativePosition(index,qtype,optcount):
	n = []
...
async def process_dropdown(q,qnum,qtype,task):
	s = []
	n = None
...
async def process_singleChoice(q,qnum,qtype,task):
	s=[]
	ans = task["q{}".format(str(qnum))]
...
async def process_contact(q,qtype,task):
	s = {}
...
async def process_questions(rq,payload,q,task):
	try:
		...
		dim = await calculateDimensions(q,qtype,optcount)
		if "dropdown" in qtype:
			d,ansid = await process_dropdown(q,qnum,qtype,task)
			if d[0]:
				relpos = d[0]
			else:
				return done
		elif "single" in qtype:
			d,ansid = await process_singleChoice(q,qnum,qtype,task)
			if d[0]:
				relpos = d
			else:
				return done
		elif "contact" in qtype:
			contact = await process_contact(q,qtype,task)
...
async def process(task):
	email = task["email"]
	p = await get_proxy()
	print(f"[{email}] Using proxy:{p['http://']}")
	head1 = {
	...
	print(f"[{email}] Fetching form.")
	async with httpx.AsyncClient(headers=head1,limits=limits,proxies=p) as s:
		r = await s.get(task["link"])
		soup = bs(r.text,"html.parser")
		
		#checks if survey is still live
		if "This survey is currently closed." in r.text:
			print(f"[{email}] Survey is closed.")
			sys.exit()
		form = soup.find("form")
		if not form:
			print(f"[{email}] form not found.")
			sys.exit()
		survey_data = form.find("input",id="survey_data").attrs["value"]
		start = int(time.time()*1000)
		time_spent = random.randint(20000,35000)
		...
		q_check=[]
		for q in qlist:
			d = await process_questions(rq,payload,q,task)
			q_check.append(d)
			if not d:
				print(f"[{email}] Task error. Skipping.")
		...
		print(f"[{email}] Sleeping {str(time_spent)}")
		await ex_wait(time_spent)
		r2 = await s.post(task["link"],files=payload,headers=head2)
		#r2.url returns URL object
		if "survey-thanks" in str(r2.url):
			print(f"[{email}] Successfuly entered!")
		else:
			print(f"[{email}] Entry failed.")
			print(f"[{email}] Status: {str(r2.status_code)}")
		# await asyncio.sleep(5)

async def main(tasks):
	tlist = []
	for i in tasks:
		tlist.append(process(i))
	await asyncio.gather(*tlist)
	


if __name__ == '__main__':
	timers = time.monotonic()
	plist = []
	try:
		...
		traceback.print_exc()
	try:
		tasks = csv.DictReader(open("tasks_n.csv","r",encoding="utf-8"), skipinitialspace=True)
		print("Tasks loaded.")
	except Exception:
	...
	print(f"Time taken:{time.monotonic() - timers}")